...
if(!function_exists('mbw_editor_ck_template')){
	function mbw_editor_ck_template($action, $data){
		...
		$item_html		.= '<script>jQuery(document).ready(function(){ckeditor = CKEDITOR.replace( "ce_content",{"bodyClass":"mb-'.$device_type.' mb-editor mb-editor-ck","customConfig": "'.MBW_PLUGIN_URL.'plugins/editors/ck/'.$config_name.'.js","filebrowserUploadUrl":"'.$admin_ajax_url.'?mode=basic&action=mb_uploader"} ); '.$editor_css.' });</script>';
		echo $item_html;	
	}
}
