...
if(!function_exists('mbw_create_board_item_panel')){
	function mbw_create_board_item_panel($args){
		...
		$name			=  trim($args['name']);
		if(!empty($args['pid'])) $pid				= $args['pid'];
		else $pid				= '';
		//모바일 pid 설정이 존재할 경우 모바일 pid로 변경
		if(mbw_get_vars("device_type")=="mobile" && !empty($args['mobile_pid'])) $pid		= $args['mobile_pid'];		

		if(!empty($args['style'])) $style				= $args['style'];
		else $style				= '';
		if(!empty($args['class'])) $class				= ' '.$args['class'];
		else $class				= '';
		if(!empty($args['field'])) $field				= trim($args['field']);
		else $field				= 'content';
