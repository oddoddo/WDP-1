...
if(!function_exists('mbw_meta_generator')){
	function mbw_meta_generator(){
		echo '<meta name="generator" content="Powered by MangBoard '.mbw_get_option("mb_version").'" />'.PHP_EOL;
	}
}
		...
		if(!empty($data['join'])) {
			...
			if(!empty($join_array)){
				foreach($join_array as $item){
					...
					$post_id			= $mdb->get_var("SELECT ".$mb_fields["board_options"]["fn_post_id"]." FROM ".$mb_admin_tables["board_options"]." where ".$mb_fields["board_options"]["fn_board_name2"]."='".$item."' limit 1");
