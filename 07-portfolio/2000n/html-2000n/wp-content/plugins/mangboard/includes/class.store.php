...
Class MStore
{
				...
				foreach ( $where_data  as $data ) {
					...
					if(!empty($data["field"]) && isset($data["value"])){
						if(strpos($data["field"], 'fn_')===0){
							...
							$field		= mbw_value_filter($data["field"]);
						}
	...
	public function set_db_options($key="",$category=""){		
		...
		if(mbw_get_trace("mbw_db_options_meta")=="" && !empty($mdb)){
			$options					= array();		
			$where_query			= "";
	...
	public function set_board_languages(){
		...
		if(mbw_get_param("mb_locale")!=""){
			...
			if($lang=='ko' || $lang=='kr' || $lang=='ko_KR') $locale		= 'ko_KR';
			else if($lang=='ja' || $lang=='jp') $locale		= 'ja';
			else if($lang=='zh' || $lang=='zh_CN') $locale		= 'zh_CN';
			else if($lang=="en") $locale		= 'en_US';
			else $locale		= 'en_US';
			...
			else if($lang=='zh' || $lang=='zh-hans' || $lang=='zh-hant') $locale		= 'zh_CN';
			else if($lang=="en") $locale		= 'en_US';
			else $locale		= 'en_US';		
		}else{
