		...
		if(mbw_get_trace("mbw_set_shortcode_options")=="" || $is_reset){
			...
			foreach($param_array as $name){
				if(isset($args[$device_type.'_'.$name])){
					mbw_set_board_option('fn_'.$name,$args[$device_type.'_'.$name]);
				}else if(isset($args[$name])){
					mbw_set_board_option('fn_'.$name,$args[$name]);
				}
			}
			//dev2 20.01.29
			$board_mode	= mbw_get_param("mode");
			if(isset($args[$device_type.'_'.$board_mode.'_layout_type'])){
				mbw_set_vars($board_mode."_layout_type",$args[$device_type.'_'.$board_mode.'_layout_type']);
				if(isset($args[$device_type.'_'.$board_mode.'_layout_class'])){
					mbw_set_vars($board_mode."_layout_class",$args[$device_type.'_'.$board_mode.'_layout_class']);
				}
			}else if(isset($args[$board_mode.'_layout_type'])){
				mbw_set_vars($board_mode."_layout_type",$args[$board_mode.'_layout_type']);
				if(isset($args[$board_mode.'_layout_class'])){
					mbw_set_vars($board_mode."_layout_class",$args[$board_mode.'_layout_class']);
				}
			}
			if(!empty($args['order_by']) && empty($_REQUEST["order_by"])){
				mbw_set_param("order_by", $args['order_by']);
				if(!empty($args['order_type']) && empty($_REQUEST["order_type"])){
					mbw_set_param("order_type", $args['order_type']);
				}
	...
	function mbw_set_search_field($args){
		if(!empty($args["search_field"]) && mbw_get_param("mode")=='list'){
			if(!empty($args["search_text"])){
				mbw_set_board_where(array("field"=>$args["search_field"], "value"=>"%".$args["search_text"]."%", "sign"=>"like"));
			}else if(!empty($args["search_value"])){
				mbw_set_board_where(array("field"=>$args["search_field"], "value"=>$args["search_value"], "sign"=>"="));
			}
		}
	}
}
