			...
			if($mode=='view' || $mode=='write'){
				...
				if(!empty($layout_type) && (empty($data["tpl"]) || $data["tpl"]!='tag')){
					if(strpos($layout_type,'responsive-box')===0){
						...
						$data["add_start_html"]		= '<div class="mb-responsive-box"><div class="mb-responsive-box-item"><div class="mb-responsive-box-label"><label for="'.$data["item_id"].'">'.$data["name"].$required_text.'</label></div><div class="mb-responsive-box-content">'.$data["add_start_html"];
	...
	function mbw_get_extension_template($data){
		...
		if($data["tpl"]=="tag"){
			...
			if(!empty($data["file_name"])){
				if(strpos($data["file_name"], 'http') !== 0){
	...
	function mbw_is_display_item($data){
		if(empty($data[mbw_get_param("board_action")])){
			...
			if($display_type=="all"){
				...
				$check_result			= true;
			}else if($display_type=="zero" && mbw_get_board_item($data["field"],false)=="0"){
				$check_result			= true;
