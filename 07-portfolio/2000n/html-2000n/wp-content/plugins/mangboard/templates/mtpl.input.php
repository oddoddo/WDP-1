	...
	function mbw_get_input_template($mode, $data){
		...
		if(strpos($item_type,'file')===0){
			if($item_type=='file_download'){
				...
				if(mbw_get_param("board_action")=="modify"){
					...
					if(($item_type=='file_image' && !empty($file)) && ((strpos($file[$mb_fields["files"]["fn_file_type"]],"image/")!==0) || (strpos($file[$mb_fields["files"]["fn_file_description"]],"ext")===0))){
		...
		//html5
			...
			for($i;$i<$count;$i++){
				...
				$template_start	.= '<label for="'.$data["item_id"].($i+1).'"><input'.$data["ext"].__STYLE($data["style"]).' type="checkbox" name="'.$t_name.'" id="'.$data["item_id"].($i+1).'" value="'.($t_data[$i]).'" '.$checked.' />'.$t_label[$i].'</label>';
			}
